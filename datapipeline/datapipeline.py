...
def get_site_index(siteID, path_to_bssid):
...
def get_all(filepath, rssi_type, path_to_s_subm, path_to_site_index, path_to_test):
    sites = [p.split("_")[0] for p in os.listdir(filepath) if p.endswith(".txt")]
    ...
    for site in sites:
        ...
        for file in site_files:
            ...
            for time_stamp, group in grouped_imu:
                ...
                temp = time_to_features.get(str(time_stamp))
                temp.append([x, y, floor])
                temp.append([acc_feat, mag_feat, gyro_feat])
                time_to_features[str(time_stamp)] = temp
            ...
            for time_stamp, group in grouped_wifi:
                ...
                if (str(time_stamp) in time_to_features):
                    time_temp = time_to_features.get(str(time_stamp))
                    time_temp[2].append(feat.values[0])
                    time_to_features[str(time_stamp)] = time_temp
                    #time_to_features[str(time_stamp)] = time_to_features.get(str(time_stamp), [[], [], feat.values[0]]) + [feat.values[0]]

                else:
                    time_to_features[str(time_stamp)] = [[x, y, floor], [], feat.values[0]]
            site_data.append( [file, [start_x, start_y], time_to_features])
...
#return generator for rssi data.
def rssi_features(rssi_type, data_path, path_to_site_index):
    path_to_data = dict()
...
if __name__ == "__main__":
    ...
    path_to_save = '../../data/data/filtered_rssi'
    path_to_site_index = '../../data/data/indices'
    gen = rssi_features("TYPE_WIFI", data_path, path_to_site_index)
    for site, data in gen:
        with open("{}/{}.pickle".format(path_to_save,site), "wb") as f:
            pickle.dump(data, f)

    path_to_save = '../../data/data/filtered_imu'
    gen = imu_data(data_path)
    for site, data in gen:
        with open("{}/{}.pickle".format(path_to_save,site), "wb") as f:
            pickle.dump(data, f)
